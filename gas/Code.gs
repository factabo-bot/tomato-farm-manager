...
// 現作の開始日は、使用回数を「その作で何回目か」で数えるための区切り（YYYY-MM-DD）。
// 定植のたびに書き換える。空のままなら全期間を数える
var MASTER_BASE_HEADERS = ["拠点ID", "拠点名", "棟区画名", "面積a", "デフォルト品目", "有効フラグ", "表示順", "現作の開始日"];
var MASTER_WORKTYPE_HEADERS = ["作業ID", "作業名", "農薬関連フラグ", "表示順", "有効フラグ"];
// 倍率候補は「/」区切りの数値（"800/1000" のように幅があるものは複数）。画面で選ばせる。
// カンマ区切りにしないのは、スプレッドシートが桁区切りの数値と解釈して
// 800,1000 が 8001000 になってしまうため。
// 調製単位は剤型による mL / g。希釈倍率目安は原文のまま残し、注記として見せる
// 「系統・IRAC/FRACコード」は人が読む用の文章。数えるのは下の3列を使う。
//   IRACコード/FRACコード … コードだけを「/」区切りで（混合剤は2つ。例 "11/M5"）
//   本剤の使用回数 … その製品の総使用回数の上限（数値）。害虫ごとの内訳はPHI目安に文章で残す
//   成分と通算回数 … 「成分名:回数」を「/」区切り。別の製品でも同じ成分なら通算される制限。
//                    例 ダコニール1000 は "TPN:6"、フォリオゴールドは "メタラキシルM:4/TPN:6"
// いずれも未確認なら空にする。空は「制限なし」ではなく「数えない」の意味
var MASTER_MATERIAL_HEADERS = [
  "薬剤ID", "薬剤名", "区分", "有効成分", "系統・IRAC/FRACコード", "登録番号",
  "主な対象病害虫", "希釈倍率目安", "倍率候補", "調製単位",
  "PHI目安", "有機JAS適合", "必要な保護具",
  "農薬登録の有無", "備考・出典", "有効フラグ",
  "IRACコード", "FRACコード", "本剤の使用回数", "成分と通算回数",
];
...
function seedMastersIfEmpty_(ss) {
  ...
  if (baseSheet.getLastRow() < 2) {
    // 現作の開始日は空のまま。定植日が決まったらシートに入れる（空なら全期間を数える）
    var baseRows = [
    ...
    baseSheet.getRange(2, 1, baseRows.length, baseRows[0].length).setValues(baseRows);
  }
  ...
  // 区分は KUBUN_PEST_CONTROL / KUBUN_FOLIAR と完全一致でないと散布区分が「その他」に落ちるので、
  // 剤型の特殊さ（くん煙剤など）は区分ではなく希釈倍率目安・備考に書く。
  // 行数は matRows.length で渡す。ここを数字で書くと剤を足したときに件数がずれる
  ...
  if (matSheet.getLastRow() < 2) {
    var matRows = [
      ["P01", "スミチオン乳剤", "殺虫剤", "MEP（フェニトロチオン）50%", "有機リン系・IRAC 1B", "",
        "チョウ目・カメムシ・アブラムシ等", "作物ごとにラベル確認", "", "mL",
        "作物ごとにラベル確認", "FALSE",
        "保護メガネ・防除用マスク・不浸透性手袋・長袖長ズボン（目安。ラベル要確認）", "TRUE",
        "農薬・防除メモ.mdより転記。2026-06-22 露地トウモロコシで使用（本人うろ覚え）", "TRUE",
        "1B", "", "", ""],
      ["P02", "BTゼンターリ顆粒水和剤", "生物殺虫剤（微生物）", "BT（アイザワイ系統）生芽胞＋結晶毒素10%", "BT剤・IRAC 11A", "",
        "鱗翅目（チョウ目）幼虫", "作物ごとにラベル確認", "", "g",
        "作物ごとにラベル確認", "TRUE",
        "マスク・手袋（目安。ラベル要確認）", "TRUE",
        "農薬・防除メモ.mdより転記。研修先での使用実績は未確認", "TRUE",
        "11A", "", "", ""],
      ["P03", "フーモン", "殺虫剤（気門封鎖剤）", "ポリグリセリン脂肪酸エステル82.5%", "気門封鎖剤（IRAC分類対象外）", "23741号",
        "ハダニ類・アブラムシ類・コナジラミ類、うどんこ病", "1000倍", "1000", "mL",
        "収穫前日まで", "FALSE",
        "マスク・手袋（目安。ラベル要確認）", "TRUE",
        "農薬・防除メモ.mdより転記。未使用", "TRUE",
        "", "", "", ""],
      ["P04", "グレーシア乳剤", "殺虫剤", "フルキサメタミド10.0%", "イソオキサゾリン系・IRAC 30", "",
        "コナジラミ類・アザミウマ類・トマトサビダニ・ハモグリバエ類・ハスモンヨトウ・オオタバコガ・トマトキバガ", "2000倍", "2000", "mL",
        "収穫前日まで（本剤2回以内）", "FALSE",
        "保護眼鏡・農薬用マスク・不浸透性手袋・ゴム長靴・長袖長ズボン（眼/皮膚刺激性あり）", "TRUE",
        "2026-07-29 野呂3の苗で使用（2000倍）", "TRUE",
        "30", "", 2, "フルキサメタミド:2"],
      ["P05", "アファーム乳剤", "殺虫剤", "エマメクチン安息香酸塩1.0%", "アベルメクチン系・IRAC 6", "19842号",
        "ハモグリバエ類・オオタバコガ・トマトサビダニ・コナジラミ類・トマトキバガ", "2000倍（使用液量100〜300L/10a）", "2000", "mL",
        "収穫前日まで（本剤5回以内）", "FALSE",
        "保護眼鏡・農薬用マスク・手袋・長袖長ズボン", "TRUE",
        "2026-08-01 野呂1〜3（2000倍）／2026-08-24 平川（2000倍）", "TRUE",
        "6", "", 5, "エマメクチン安息香酸塩:5"],
      ["P06", "フォリオゴールド", "殺菌剤", "メタラキシルM 3.3%＋TPN 32.0%", "メタラキシルM＝FRAC 4／TPN＝FRAC M5", "",
        "葉かび病・疫病", "葉かび病800倍／疫病800〜1000倍", "800/1000", "mL",
        "収穫前日まで（各病害4回以内）", "FALSE",
        "農薬用マスク・手袋・不浸透性防除衣・保護クリーム・保護眼鏡", "TRUE",
        "2026-07-29・2026-08-01 の防除で使用（800倍）", "TRUE",
        "", "4/M5", 4, "メタラキシルM:5/TPN:6"],
      ["P07", "ドライバー", "展着剤", "ポリオキシエチレン脂肪酸エステル24.0%", "（展着剤のため分類なし）", "24222号",
        "（展着剤のため対象なし）", "1000倍（登録は1000〜5000倍）", "1000", "mL",
        "（展着剤のため設定なし）", "FALSE",
        "手袋・長袖長ズボン（眼への刺激性あり）", "TRUE",
        "研修先の混用でほぼ毎回入る。農薬登録があるので法定帳簿の対象", "TRUE",
        "", "", "", ""],
      ["P08", "クロロゲン（青）", "葉面散布肥料", "N4.0・P2.5・K2.0＋微量要素（玉名化学）", "（肥料のため分類なし）", "",
        "（肥料のため対象なし）", "500倍", "500", "mL",
        "（肥料のため設定なし）", "FALSE",
        "", "FALSE",
        "肥料登録のため法定帳簿の対象外。赤（K型）・黄（P-K型）は未使用", "TRUE",
        "", "", "", ""],
      ["P09", "アクタラ顆粒水溶剤", "殺虫剤", "チアメトキサム10.0%", "ネオニコチノイド系・IRAC 4A", "20672号",
        "コナジラミ類", "2000倍（トマト・コナジラミ類）", "2000", "g",
        "収穫前日まで（本剤3回以内）", "FALSE",
        "保護眼鏡・手袋・長袖長ズボン（メーカー記載。ラベル要確認）", "TRUE",
        "2026-08-12 野呂3の防除で使用。倍率は指示書に記載なし", "TRUE",
        "4A", "", 3, "チアメトキサム:4"],
      ["P10", "トリフミン水和剤", "殺菌剤", "トリフルミゾール30.0%", "DMI（イミダゾール）系・FRAC 3", "",
        "葉かび病・すすかび病・うどんこ病", "3000倍（トマト）", "3000", "g",
        "収穫前日まで（本剤の回数は資料により3回／5回。要確認）", "FALSE",
        "農薬用マスク・不浸透性手袋・長袖長ズボン（使用後は洗浄・うがい・着替え）", "TRUE",
        "2026-08-12 野呂3の防除で使用。正式登録名は石原トリフミン水和剤。倍率は指示書に記載なし", "TRUE",
        "", "3", "", ""],
      ["P11", "アミスターオプティフロアブル", "殺菌剤", "アゾキシストロビン5.1%＋TPN 40.0%", "アゾキシストロビン＝FRAC 11／TPN＝FRAC M5", "22144号",
        "疫病・葉かび病・炭疽病・灰色かび病・すすかび病・斑点病", "1000倍（トマト）", "1000", "mL",
        "収穫前日まで（本剤4回以内）", "FALSE",
        "ラベル原本を未確認。使用前に必ずラベルで確認", "TRUE",
        "2026-08-24 平川の防除で使用（1000倍）", "TRUE",
        "", "11/M5", 4, "アゾキシストロビン:4/TPN:6"],
      ["P12", "ベネビアOD", "殺虫剤", "シアントラニリプロール10.3%", "ジアミド系・IRAC 28", "24091号",
        "オオタバコガ・ハモグリバエ類・コナジラミ類・アザミウマ類・アブラムシ類・トマトキバガ", "2000倍（オオタバコガのみ2000〜4000倍）", "2000/4000", "mL",
        "収穫前日まで（本剤3回以内・シアントラニリプロール通算5回以内）", "FALSE",
        "ラベル原本を未確認。使用前に必ずラベルで確認", "TRUE",
        "2026-08-31 平川の防除で使用（2000倍）", "TRUE",
        "28", "", 3, "シアントラニリプロール:5"],
      ["P13", "ダコニール1000", "殺菌剤", "TPN 40.0%", "クロロニトリル系（多作用点）・FRAC M5", "16823号",
        "疫病・輪紋病・葉かび病・炭疽病・灰色かび病・すすかび病・うどんこ病・褐色輪紋病・斑点病", "1000倍（トマト）", "1000", "mL",
        "収穫前日まで（本剤4回以内・TPNを含む剤で通算6回以内）", "FALSE",
        "ラベル原本を未確認。使用前に必ずラベルで確認", "TRUE",
        "2026-08-31 平川・2026-09-18 野呂の防除で使用（1000倍）", "TRUE",
        "", "M5", 4, "TPN:6"],
      ["P14", "ウララDF", "殺虫剤", "フロニカミド10.0%", "フロニカミド・IRAC 29", "21812号",
        "アブラムシ類・コナジラミ類・ミカンキイロアザミウマ", "2000〜4000倍（アブラムシ類・コナジラミ類）／2000倍（ミカンキイロアザミウマ）", "2000/4000", "g",
        "収穫前日まで（散布は本剤3回以内）", "FALSE",
        "眼への刺激性あり。常温煙霧の最中はハウスに入らない（ラベル要確認）", "TRUE",
        "2026-09-11 野呂の防除で使用（2000倍）", "TRUE",
        "29", "", 3, "フロニカミド:3"],
      ["P15", "ファンタジスタ顆粒水和剤", "殺菌剤", "ピリベンカルブ40.0%", "ベンジルカーバメート系・FRAC 11A（コード11のG143A変異株とは交差耐性なし）", "23103号",
        "灰色かび病・菌核病・葉かび病・すすかび病・斑点病・立枯病", "2000〜3000倍（立枯病のみ2000倍）", "2000/3000", "g",
        "収穫前日まで（本剤3回以内）", "FALSE",
        "ラベル原本を未確認。使用前に必ずラベルで確認", "TRUE",
        "2026-09-11 野呂の防除で使用（2000倍）。正式登録名は日曹ファンタジスタ顆粒水和剤", "TRUE",
        "", "11A", 3, "ピリベンカルブ:3"],
      ["P16", "クリアザールフロアブル", "殺虫剤", "スピロメシフェン22.9%", "テトロン酸系・IRAC 23", "22093号",
        "コナジラミ類・トマトサビダニ", "2000〜4000倍（コナジラミ類）／2000倍（トマトサビダニ）", "2000/4000", "mL",
        "収穫前日まで（本剤2回以内）", "FALSE",
        "ラベル原本を未確認。使用前に必ずラベルで確認", "TRUE",
        "2026-09-18 野呂の防除で使用（2000倍）。指示書の表記は「クリアザール」で、この登録剤と同じかは未確認", "TRUE",
        "23", "", 2, "スピロメシフェン:2"],
      ["P17", "パルミノフロアブル", "殺虫殺菌剤", "キノメチオナート25.0%", "キノキサリン系・FRAC M10／IRAC UN", "23587号",
        "うどんこ病・コナジラミ類・トマトサビダニ", "2000倍（トマト）", "2000", "mL",
        "収穫前日まで（本剤5回以内）", "FALSE",
        "ラベル原本を未確認。使用前に必ずラベルで確認", "TRUE",
        "2026-09-18 野呂の防除で使用（2000倍）。登録上の種類名は「パルミノ」", "TRUE",
        "", "M10", 5, "キノメチオナート:5"],
      ["P18", "モスピランジェット", "殺虫剤", "アセタミプリド15.0%", "ネオニコチノイド系・IRAC 4A", "19326号",
        "コナジラミ類・ミカンキイロアザミウマ", "希釈しない。室容積400立方メートル（床面積200平方メートル×高さ2m）あたり50g", "", "g",
        "収穫前日まで（本剤3回以内）", "FALSE",
        "農薬用マスク・手袋・長袖長ズボン（参考情報。ラベル原本は未確認）", "TRUE",
        "くん煙剤（劇物）。2026-08-24までに平川でくん煙。自分は未実施", "TRUE",
        "4A", "", 3, "アセタミプリド:4"],
    ];
    matSheet.getRange(2, 1, matRows.length, matRows[0].length).setValues(matRows);
  }
